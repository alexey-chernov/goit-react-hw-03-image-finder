...
class App extends Component {
  ...
  renderImages = async () => {
    ...
    if (!query.trim()) {
      return toast.info('Будь ласка, введіть значення для пошукових зображень! ');
    }
    ...
    try {
      ...
      if (request.length === 0) {
        this.setState({ error: `Не знайдено результатів для ${query}!` });
      }
    } catch (error) {
      this.setState({ error: 'Щось пішло не так. Спробуйте ще раз. ' });
    } finally {
      this.toggleLoader();
    }
  };
